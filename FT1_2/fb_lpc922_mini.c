...
unsigned char __idata __at (0xFE-23) s_telegramm[23];
unsigned char  __idata __at (0xFE-46) r_telegramm[23];
...
__bit send_ack,send_busy, send_nack, transparency;
...
void T1_int(void) __interrupt (3) 	// Timer 1 Interrupt
{
	...
	case 4:	//	Timeout, d.h. Telegramm-Ende
		if (auto_ack && telpos>7) {		// wenn ACK gesendet werden soll und Telegramm zumindest 5 Bytes hat
			...
			if(r_telegramm[5]&0x80) {
				if (gapos_in_gat(r_telegramm[3],r_telegramm[4])!=0xFF) its_me=1;	// Gruppenadresse
				if (r_telegramm[3]==0 && r_telegramm[4]==0) its_me=1;				// Broadcast
			}
			else if(r_telegramm[3]==eeprom[ADDRTAB+1] && r_telegramm[4]==eeprom[ADDRTAB+2]) its_me=1;	// phys. Adresse
			//checken ob Telegramm ok ist
			if ((cs==0xFF)&&(((r_telegramm[5]&0x0F)+8)== (telpos)))
			{
				telegramm_ok=1;
				if(its_me)tel_arrived=1;
			}
	...
	case 10:	// Byte Senden, T=0us
		send_byte=s_telegramm[s_telpos];
		if (send_ack) send_byte=0xCC;
		if (send_nack) send_byte=0x0C;
		if (send_busy)send_byte=0xC0;
	...
	case 11:	// Sendestufe aus, T=35us
		if(s_telpos==0 && fb_pattern==1) {	// 1. Byte Startbit
...
*/
	case 13:	// Byte übertragen, T=35us, prüfen ob Telegramm fertig
		...
		if (send_ack || send_nack) {	// ACK/NACK senden
			...
			if (s_telpos>((s_telegramm[5]&0x0F)+7)) {		// Telegramm fertig gesendet
				tel_sent=1;
...
{
	unsigned char n, cs_pos, cs_lokal;

	TR1=0;
	TMOD=(TMOD & 0x0F) +0x20;			// Timer 1 als 8-Bit autoreload
	TH1=110;							//110 von state 10 auf 11
	TL1=128;							// von hier bis state 10
	TF1=0;

	cs_pos=(s_telegramm[5]&0x0F)+7;		// Position der Checksum im Telegramm
	cs_lokal=0xFF;
	for(n=0;n<cs_pos;n++) {
		cs_lokal^=s_telegramm[n];				// Checksum berechnen
	}
	s_telegramm[cs_pos]=cs_lokal;				// Checksum hinter Nutzdaten anfügen
	ack=0;
	nack=0;
	busy=0;
	wait_for_ack=1;
	...
	s_telpos=0;							// nächstes zu sendende Byte
...
void process_tel(void)
{
	...
	tel_arrived=0;
	tpdu=r_telegramm[6]&0xC3;
	apdu=r_telegramm[7];

	// Broadcast,
	if(r_telegramm[3]==0 && r_telegramm[4]==0) {	// nur wenn wenn Zieladresse = 0
...
*/	}
		...
		if((r_telegramm[5]&0x80)==0x00) {	// Destination Adress Flag Bit 7, 0=phys. Adr., 1=Gruppenadr.
