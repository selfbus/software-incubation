...
const unsigned int luxdelay[] = {0,15,38,75,112,149,225,337,449,674,897,1346,2243,4485,6738,13454};
...
void schwelle(unsigned char objno)		// Luxschwelle prüfen und reagieren
{
	...
	if ((eeprom[LUXPARAM]&0x80) && objno==3) {	// Helligkeitsregelung (nur Objekt1)
		...
		if (objno<6) {	// Helligkeitsschwellen
			...
			case 3:
				if ((eeprom[LUXPARAM]&0x80) == 0) {
					if (under) delay=luxdelay[(eeprom[0xCD] & 0xF0) >> 4];
				}
				else delay=0;	// bei Helligkeitsregelung keine Verzögerung
				break;
			case 4:
				if (under) delay=luxdelay[eeprom[0xCD] & 0x0F];
				break;
			case 5:
				if (under) delay=luxdelay[(eeprom[0xCC] & 0xF0) >> 4];
				break;
			}
			...
			if (objno<8) {	// Temperaturschwellen
				...
				case 0x10:		// 1 = größer als Helligkeitsschwelle 1
					if(delrec[2].delayactive & 0x04) valid=0;	// wenn Verzögerung der Helligkeitsschwelle noch aktiv, dann noch nichts senden
					over &= (overrun & 0x01);
					break;
				case 0x20:		// 2 = größer als Helligkeitsschwelle 2
					if(delrec[3].delayactive & 0x04) valid=0;
					over &= (overrun & 0x02);
					break;
				case 0x30:		// 3 = größer als Helligkeitsschwelle 3
					if(delrec[4].delayactive & 0x04) valid=0;
					over &= (overrun & 0x04);
					break;
				case 0x40:		// 4 = kleiner als Helligkeitsschwelle 1
					if(delrec[2].delayactive & 0x04) valid=0;
					over &= (underrun & 0x01);
					break;
		...
		if (over) value = eeprom[LUX_OVER-offset];		// zu sendender Wert bei überschreiten
		else value = eeprom[LUX_UNDER-offset];		// zu sendender Wert bei unterschreiten
		...
		if (over) {					// wenn überschritten
			if (!(overrun&bitmask)) { 	// wenn neu überschritten
				...
				if (delay==0 || (resend&bitmask)) {		// // wenn keine Verzögerung oder Sperre aufgehoben wurde, sofort senden
					resend&=~bitmask;
					active=3;	// Bit0=1=aktiv und Bit1=1=überschritten
				}
				else {	// verzögert senden
					if(delrec[objno-1].delayactive!=5) {	// wenn bereits verzögertes unterschreiten läuft timer inaktiv setzen
		...
		if (under) {										// wenn Wert unter Schwellwert
			if (!(underrun&bitmask)) { 						// wenn neu unterschritten
				...
				if (delay==0 || (resend&bitmask)) {	// wenn keine Verzögerung oder Sperre aufgehoben wurde, sofort senden
					...
					if(delrec[objno-1].delayactive!=7) {	// falls verzögertes überschritten Senden noch läuft stoppen
