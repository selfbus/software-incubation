...
unsigned char __idata __at(0xFE-16)timercnt[16];
unsigned char __idata __at(0xFE-32)timerbase[16];
unsigned char __idata __at(0xFE-48)timerstate[16];
unsigned char bit_obj[8];
__code unsigned char schrittweite[11] ={1,3,5,8,10,13,15,18,20,29,25};
/** 
...
void button_changed(unsigned char buttonno, __bit buttonval)
{
	...
	switch ((eeprom[FUNCTION+(buttonno>>1)] >> ((buttonno & 0x01)*4)) & 0x0F) {		// Funktion des Tasters (0xCF)
	...
	case 2:
		...
		if (buttonval) {	// Taster gedrueckt -> schauen wie lange gehalten
			...
			timercnt[buttonno+8]=eeprom[0xD6+(buttonno*4)];	// Faktor Dauer			

			timerbase[buttonno+8]=(eeprom[0xD5+(buttonno*4)]&0x70)>>4;// Basis Dauer zwischen kurz und langzeit
			...
			if (bedienung==0x10){	// heller
				timerstate[buttonno+8]= ((eeprom[0xD4+(buttonno*4)]&0xF0)>>4)+0x20;	// dimmen
			}
			if(bedienung==0x30){	//  dunkler
				timerstate[buttonno+8]= ((eeprom[0xD4+(buttonno*4)]&0x0F))+0x20;	// dimmen
			}
				
			
		}
		else {		// Taster losgelassen
			if ((timerstate[buttonno+8]& 0xF0)==0x20) { // wenn delaytimer noch lauft, dann Schalten, also EIS1 telegramm senden
			...
			timercnt[buttonno+8]=0;
			timerstate[buttonno+8]=0;
		}
		break;
	...
	case 3:
		if (buttonval) {	// Taster gedrueckt -> schauen wie lange gehalten
			...
			timercnt[buttonno+8]=eeprom[0xD5+(buttonno*4)];	// Faktor Dauer			
			timerbase[buttonno+8]=(eeprom[0xD4+(buttonno*4)]&0x06)*2;// Basis Dauer zwischen kurz und langzeit
			timerstate[buttonno+8]=((eeprom[0xD3+(buttonno*4)]&0x10)>>4)+0x30;
		}
		else {	// Taster losgelassen
			if (timerstate[buttonno+8] >= 0x40) {//innerhalb T2
				write_obj_value(buttonno,((eeprom[0xD3+(buttonno*4)]&0x10)>>4));
				send_obj_value(buttonno);
			}
			else{
				//timercnt[buttonno+4]=0; der ist bereits 0	
				timerstate[buttonno+8]=0;// T2 bereits abgelaufen
			}
		}
		break;
	...
	case 4:	// Wertgeber..
		...
		case 1:	// ++++++++ Lichtszenenabruf mit Speicherfunktion
			if(buttonval){// beim druecken
				...
				timercnt[buttonno+8]=156;
				timerbase[buttonno+8]=2; //(32ms)
				timerstate[buttonno+8]=0x50;// Betaetigung lïaenger 5 sekunden bei Lichtszene

			}
			else{// nach loslassen...
				if (timerstate[buttonno+8]==0x50){// wenn 5 sekunden noch nicht erreicht LZ senden
					write_obj_value(buttonno+8,eeprom[0xD5+(buttonno*4)]);
					switch_led(buttonno,1);
				}
		...
		case 2: // ++++++  Helligkeitswertgeber
			if (buttonval){
				...
				if(!(eeprom[0xD3+(buttonno*4)]& 0x80)){// schauen ob Verstellung freigegeben
					timercnt[buttonno+8]=156;
					timerbase[buttonno+8]=2; //(32ms)
					timerstate[buttonno+8]=0x70;// Betï¿½tigung lï¿½nger 5 sekunden bei Dimmwert
				}
			}
			else{
				if (timerstate[buttonno+8]==0x80){
		...
		case 3:	// ++++++  Temperaturwertgeber
			if (buttonval){
				...
				if(!(eeprom[0xD3+(buttonno*4)]& 0x80)){// schauen ob Verstellung freigegeben
					timercnt[buttonno+8]=156;
					timerbase[buttonno+8]=2; //(32ms)
					timerstate[buttonno+8]=0x90;// Betaetigung laenger 5 sekunden bei Dimmwert
				}
			}
			else{
				if (timerstate[buttonno+8]==0xA0){
					//dimmwert abspeichern kommt noch
		...
		case 4: // ++++++   Dimmwertgeber
			if (buttonval){
				...
				if(!(eeprom[0xD3+(buttonno*4)]& 0x80)){// schauen ob Verstellung freigegeben
					timercnt[buttonno+8]=156;
					timerbase[buttonno+8]=2; //(32ms)
					timerstate[buttonno+8]=0xB0;// Betaetigung laenger 5 sekunden bei Dimmwert
				}
			}
			else{
				if (timerstate[buttonno+8]==0xC0){
					//dimmwert abspeichern kommt noch
...
int eis5conversion(int zahl,unsigned char Typ)
{
	...
	if (Typ==6){// Temperaturwert kleinster wert =1 grï¿½ï¿½ter 31
		
		wert=zahl*100;// Hier reicht uns eine 16bit int var
	}
	if (Typ==7){// wenn Dimmwert ( EIS2, also keine Flieï¿½komma)
		wert=zahl;
	}
...
#ifdef TASTER_8
	if(objno<8)	return(bit_obj[objno]&0x01);
...
{
#ifdef TASTER_8
	if(objno<8)bit_obj[objno]=objval&0x01;
	else object_value[objno-8]=objval;
#else
...
void write_value_req(unsigned char objno)
{
	...
	if (objno<16) {					// max 16 objekte (0-11)
		...
		if ((objno<8) && ((eeprom[COMMAND+(objno*4)]) & 0x07) <4) switch_led(objno,telegramm[7]&0x01);	// LED nur schalten, wenn nicht auf Betaetigungsanzeige parametriert
...
void timer0_int  (void) __interrupt (1) // Interrupt T0 fï¿½r soft PWM LED
{
  dimmcompare++;
  TF0=0;
#ifndef TASTER_8
...
// +++++++  Hier werden alle timer  gemaess ihrer basis decremntiert
	...
	for(n=0;n<16;n++){
		if(timerflags & 0x0001){// positive flags erzeugen und schieben
			for(m=0;m<16;m++){// die timer der reihe nach checken und dec wenn laufen
				if( timercnt[m]){
	...
	for(objno=0;objno<16;objno++) {
		if(!timercnt[objno]) {	// ... es ist also soweit
			...
			case 0x10:// kommt nur in den timern 0-7 vor
			// LED bei Betaetigungsanzeige nach eingestellter Zeit ausschalten
		    ledvar=LEDSTATE;
			...
			// alle anderen kommen nur in den timern 8-15 vor
			case 0x20:	// Dimmen
				write_obj_value(objno,delay_value);
				send_obj_value(objno);		// dimmkommando senden

				if(!(eeprom[0xD5+((objno-8)*4)]& 0x08)){// wenn Telegrammwiederholung eingeschalten
...
*/				timercnt[objno]=tele_repeat_value[eeprom[0xD5+((objno-8)*4)]& 0x07];	
//				timercnt[objno]=tmp;
				timerbase[objno]=0;
				}
				else{
			...
			case 0x30:	// 0x30, 0x31 fuer langzeit telegramm senden
						write_obj_value(objno,delay_value & 0x01);
						send_obj_value(objno);				// Langzeit Telegramm senden

						// *** timercnt neu laden fuer Dauer Lamellenverstellung ***
						timercnt[objno]=eeprom[DEL_FACTOR2+((objno-8)*4)];	// Faktor Dauer	T2		
						timerbase[objno]=(eeprom[0xD4+((objno-8)*4)]&0x60)>>3;	// Basis Dauer T2
						if (timercnt[objno]) {	// wenn keine lamellenverstellzeit dann nix tun
							timerstate[objno]=((eeprom[COMMAND+((objno-8)*4)]&0x10)>>4)+0x40; // 0x40,0x41 fuer ende T2 (lamellenvestellzeit)
						}
						else {
							timercnt[objno]=0;
							timerstate[objno]=0;
			...
			case 0x80: // Wertgeber Helligkeit verstellen
				timerbase[objno]=0;
				timercnt[objno]=tele_repeat_value[(eeprom[0xD4+((objno-8)*4)]& 0x30)>>4];// schrittweite und zeit zw. telegrammen
				i_tmp=(eeprom[0xD5+4*(objno-8)])<<8;
				i_tmp +=eeprom[0xD6+4*(objno-8)];
				...
				write_obj_value(objno,i_tmp);// wir rechnen immer mit exp 2^7
				timerstate[objno]=0x80;
    			EA=0;		// Interrupts sperren, damit flashen nicht unterbrochen wird
    			START_WRITECYCLE
    			WRITE_BYTE(0x01,0xD5+((objno-8)*4),read_obj_value(objno)>>8);
    			WRITE_BYTE(0x01,0xD6+((objno-8)*4),read_obj_value(objno)& 0xFF);
			...
			case 0xA0: // Wertgeber Temperatur verstellen
				timerbase[objno]=0;
				timercnt[objno]=tele_repeat_value[(eeprom[0xD4+((objno-8)*4)]& 0x30)>>4];// schrittweite und zeit zw. telegrammen
				i_tmp=(eeprom[0xD5+4*(objno-8)]&0x07)<<8;
				i_tmp +=eeprom[0xD6+4*(objno-8)];
				if(i_tmp>=0x0032){
					i_tmp -= 0x32;
					switch_led(objno-8,1);
				...
				write_obj_value(objno,i_tmp|0x0800);// wir rechnen immer mit exp 2^1
			...
			case 0xC0: // Wertgeber Dimmer verstellen
					tmp=read_obj_value(objno);//
					m=eeprom[0xD4+((objno-8)*4)]& 0x3F;// schrittweite und zeit zw. telegrammen
					n= schrittweite[m & 0x0F];  // Schrittweite
...
#ifdef TASTER_8
#	ifdef HIGH_ACTIV
		P0=0x00;			// Taster auf high
		button_buffer=0x00;	// Variable fuer letzten abgearbeiteten Taster Status
#	else
		P0=0xFF;			// Taster auf high
		button_buffer=0xFF;	// Variable fuer letzten abgearbeiteten Taster Status
#	endif
#else
...
//	STOP_WRITECYCLE;
	...
	for (n=0;n<16;n++)
		{
		timercnt[n]=0;		// delay records loeschen
		timerstate[n]=0;
		}
