...
void tastenauswertung(void)
{
  ...
  if(P1_3==0)           //abfrage Taste getrückt
  ...
  else
    {
      if(mtaste[ctaste]>20)
        {
          if(mtaste[ctaste]<200) //kurzer tastendruck mit Tastenprellung
            {
          ...
          else //langer tastendruck
          	{
            if(ctaste==1||ctaste==2) dimmen_obj(0,0);
            if(ctaste==5||ctaste==6) dimmen_obj(1,0);
          	}
        }
      mtaste[ctaste]=0;
    }
