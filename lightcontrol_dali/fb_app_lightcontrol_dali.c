...
__idata __at (0xFE-3)unsigned char D_hell[3];
...
unsigned char D_position;// Position im Manchester code
unsigned char D_f_busy;// Dali state beschäftigt

const unsigned char grundhelligkeit_tabelle[]={15,30,46,61,77,92,107,127};
const unsigned char prozentvalue[]={25,51,75,102,128,154,179,204,230,254};
...
void timer0_int  (void) interrupt 1 {// Interrupt T0 für soft PWM LED
  ...
  if (dimmcompare !=255)
  {
    ...
    // if (dimmcompare==dimmwert[3]) P0_3=0;
  }
...
//  else P0_3=0;
  ...
  dimmtimervorteiler++;
  dimmtimervorteiler &= 0x07;
  if (!dimmtimervorteiler){			//vorteiler wird alle 50µs in softpwm erhöht
		if (D_f_busy){
			//send_frame();			// abarbeiten der DALI Satemachine
			
			  if (D_value)   P0 &= bitmask_0[D_f_busy-1];                // DALI output pin high 
			  else     		P0 |= bitmask_1[D_f_busy-1];                // DALI output pin low 
			   
			          if (D_position == 0)                       // 0TE second half of start bit = 1 
			          { 
			              D_value = 1; 
			          } 
			          else{
			        	  if (D_position < 33)                  // 1TE - 32TE, so address + command 
				          { 
				              D_value = D_forward  & 0x8000;
				              if (D_position & 1){ 
				                D_value = !D_value;                  // invert if first half of data bit 
				              }
				              else	D_forward <<= 1;

				          } 
			        	  if (D_position == 33)                 // 33TE start of stop bit (4TE) 
			        	  {                                        // and start of min settling time (7TE) 
			        		  D_value = 1; 
			        	  } 
			        	 // if (D_position == 44)                 // 44TE, end stop bits + settling time 
			        	 // {                                        // receive slave answer, timeout 
			        		  //  T0MR0 = 9174;                        // of 22TE = 9,174 msec 
			        		  //  T0CCR = 0x0007;                      // enable rx, capture on both edges 
			        	 // } 
			        	  if (D_position == 100)   //80  original 45             // end of transfer 
			        	  { 
			              //T0TCR = 2;                           // stop and reset timer 
			          //    if (D_frame & 0x100)                   // backward frame (answer) completed ? 
			          //    { 
			                  // D_answer = (BYTE)frame;           // OK ! save answer 
			          //         D_f_dalirx = 1;                   // and set flag to signal application 
			          //    } 
			           //   D_frame  = 0;                          // reset receive frame 
			              D_f_busy = 0;                          // end of transmission 
  ...
  }// ergibt eine flanke von 0.52ms(entspricht 2400bd für 1200bd manchester code)
...
void hell_stellen (unsigned char obj,unsigned char value){
	...
	if (value){// wenn Helligkeit >0
		...
		if (value>=grundhelligkeit[obj]){
	   		dimmziel[obj]=value;
	   		//dimmziel[obj]=((value * (255 - grundhelligkeit[obj])) /255)+ grundhelligkeit[obj];
		}
		else{
...
void port_schalten(void)		// Schaltet die Ports 
{
	...
	for(n=0;n<3;n++){
		if(pattern&bitmask_1[n]){
			if(schalten & bitmask_1[n]){
				DALI_send(0xFE00+dimmpwm[n],n);
				//dimmwert[n]=0;
				//dimmziel[n]=0;
				//dimmpwm[n]=0;
				//rs_send(0xFF);
...
void send_frame(void)
{
  ...
  { 
      do{
          while (D_f_busy ) ;                             // Wait until dali port is idle 
	      D_forward=D_send_value;
	      D_value    = 0;                                // first half of start bit = 0 
	      D_position = 0; 
	      while(fb_state);
	      interrupted=0;
	      D_f_busy   = Kanal+1;                                // set transfer activate flag 
	    //  rs_send(D_send_value>>8);rs_send(D_send_value);
...
//	WRITE_BYTE(0x01,0x12,0x8A)	// COMMSTAB Pointer
	...
	TH0=0x40; //47
	TL0=0x40; //47 bei 50µs  40 bei 52µ für 2400bd
	TR0=1;
