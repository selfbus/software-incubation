...
void write_value_req(unsigned char objno) 				// Ausgänge schalten gemäß EIS 1 Protokoll (an/aus)
{
 ...
 // unsigned char blockstart, blockend, block_polarity;
  ...
  unsigned char objval=telegramm[7]&0x01;        
           obj_bitpattern=0x01<<(objno-8);
          if (objno<8) object_schalten(objno,objval);	// Objektnummer 0-7 entspricht den Ausgängen 1-8

          if (objno>7 && objno<12)	// Objektnummer 8-11 entspricht den Zusatzfunktionen 1-4
          {
      		if(objno==8 && (!(portbuffer&0x01))&&objval){// objekt 8 startet Lauflicht aufwärts
      			pattern_up |=1;
      			portbuffer|=1;
      			timercnt[8]=eeprom[0+0xE2]|0x80;// weiterschaltzeit setzen
				timercnt[0]=eeprom[2+0xE2]|0x80;// Ausschaltzeit des 1. Ausganges setzen
				timerbase[0]=eeprom[DELAYTAB+1]>>4;
      		}
      		if(objno==9 && (!(portbuffer&0x80))&& objval){// objekt 9 startet lauflicht abwärts
      			pattern_down|=0x80;
      			portbuffer|=0x80;
      			timercnt[9]=eeprom[1+0xE2]|0x80;
				timercnt[7]=eeprom[3+0xE2]|0x80;
				timerbase[7]=eeprom[DELAYTAB+1]&0x0F;
      		}
    		write_obj_value(objno,objval);		// Objektwert speichern
         }
...
void object_schalten(unsigned char objno, __bit objstate)	// Schaltet einen Ausgang gemäß objstate und den zugörigen Parametern
{
	...
	delay_base;delay_state;logicfunc;zfno;
	delay_onoff;
...
*/
		if(((0x01<<objno) & blocked)==0 ) {	// Objekt ist nicht gesperrt und Kommunikation zulässig (Bit 2 = communication enable) und Schreiben zulässig (Bit 4 = write enable)
			//delay_base=eeprom[(((objno+1)>>1)+DELAYTAB)];
			delay_base=eeprom[DELAYTAB+1]>>4;// immer die basis von Obj 2 nehmen.
			...
			if ( objstate==0 )
			{
				//delay_onoff=0;//eeprom[2+0xE2];// faktor zuweisen immer Obj 2
			...
			if ( objstate==1  )
			{
				delay_onoff=eeprom[2+0xDA];
				if(delay_onoff==0x00) {

/*
					if(((eeprom[0xEA]>>objno)&0x01)==0x01) { 			// Zeitschaltfunktion
						delay_state=0x80;
						delay_onoff=eeprom[2+0xE2];
					} 
